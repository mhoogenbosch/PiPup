...
| **`rtsp://` video** (since 0.15.0/0.15.1) | **6.0.1+**, via ExoPlayer with RTP-over-TCP, rendered in a `TextureView` — so it also shows **over video the TV is already playing** (verified on a Fire TV with a film running). Audio of the stream is not played. |
| `http` / HLS `video_url` and `camera_mode: stream` | all versions, via the stock `VideoView` — but that is a SurfaceView on the hardware video plane, so it **can contend with video already playing** on constrained TVs (black popup or interrupted playback). Prefer `rtsp://`, or MJPEG/WebRTC via `web_url`, on a TV that is watching something. |
